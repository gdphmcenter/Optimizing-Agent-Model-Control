...
for i =1:1000
        ...
        u1=test([e(i),ec])*Ta(3)*test2(e2(i),de)+disturb;
        u2=test([e(i),ec])*Ta(4)*test2(e2(i),de)+disturb;
        u3=test([e(i),ec])*Ta(5)*test2(e2(i),de)+disturb;
        u4=test([e(i),ec])*Ta(6)*test2(e2(i),de)+disturb;
        acc1=acc1+u1;
        acc2=acc2+u2;
        acc3=acc3+u3;
        acc4=acc4+u4;
...
z=fitadd;
figure(1);plot(chu,'LineWidth', 1.1 ...
    );
hold on
...
for i =1:1000
        ...
        u1=test([e(i),ec])*Ta(3)+disturb;
        u2=test([e(i),ec])*Ta(4)+disturb;
        u3=test([e(i),ec])*Ta(5)+disturb;
        u4=test([e(i),ec])*Ta(6)+disturb;
        acc1=acc1+u1;
        acc2=acc2+u2;
        acc3=acc3+u3;
        acc4=acc4+u4;
...
chu=ones(1, 1000);
figure(1);plot(chu,'LineWidth', 1.1);
legend('基于QNPSO的空调温度动态模糊控制', '基础增量模糊控制','PSO优化增量模糊控制', 'GA优化增量模糊控制','QNPSO优化增量模糊控制','设定温度增量');
% 获取图例句柄  
l = legend;  
  
% 设置图例的字号  
set(l, 'FontSize', 10); % 将字号设置为12，你可以根据需要调整这个值
xlabel('time(s)','FontSize',12);  
ylabel('出风温度设定增量(°C)','FontSize',12); 
